...
import { Plus, Edit, Trash2, Eye, Send, X, Save, Mail } from 'lucide-react'
...
export default function AdminEmailTemplatesPage() {
        ...
        const seed: EmailTemplate[] = [
          {
            ...
            name: 'ยืนยันการจอง',
            subject: 'ยืนยันการจองของคุณ - {{serviceName}}',
            body:
              'สวัสดี {{userName}},\n\nการจองของคุณสำหรับ {{serviceName}} วันที่ {{date}} ได้รับการยืนยันแล้ว ยอดชำระ {{amount}} บาท\n\nขอบคุณที่ใช้บริการ ChiangMaiGo',
          ...
          },
          {
            ...
            body:
              'เรียน {{userName}},\n\nกดลิงก์ด้านล่างเพื่อรีเซ็ตรหัสผ่านของคุณ:\n{{resetLink}}\n\nหากคุณไม่ได้ร้องขอ โปรดเพิกเฉยอีเมลนี้',
            updatedAt: new Date().toISOString(),
          },
        ]
        setTemplates(seed)
  ...
  const persist = (list: EmailTemplate[]) => {
    setTemplates(list)
    if (typeof window !== 'undefined') {
  ...
  const preview = (t: EmailTemplate) => {
    const sample = t.body
      .replaceAll('{{userName}}', 'คุณสมชาย')
      .replaceAll('{{serviceName}}', 'ทัวร์ดอยสุเทพ')
      .replaceAll('{{date}}', '2025-12-01')
      .replaceAll('{{amount}}', '1,500')
      .replaceAll('{{resetLink}}', 'https://example.com/reset?token=demo')
    alert(`ตัวอย่างอีเมล:\n\nSubject: ${t.subject}\n\n${sample}`)
  }

  const sendTest = (t: EmailTemplate) => {
    const email = prompt('กรอกอีเมลปลายทางสำหรับทดสอบ', 'user@example.com')
    if (!email) return
    // เดโม: เพียงแจ้งเตือนแทนการส่งจริง
    alert(`ส่งทดสอบแม่แบบ "${t.name}" ไปยัง ${email} (เดโม)`)
  }
    ...
    <div className="min-h-screen bg-neutral-50">
      <div className="container mx-auto px-4 py-8">
        <div className="flex items-center justify-between mb-8">
          <h1 className="text-3xl font-bold">แม่แบบอีเมล</h1>
        ...
        <>
          ...
          <aside className="fixed right-0 top-0 h-full w-full md:w-[520px] bg-white z-50 shadow-xl border-l border-neutral-200 flex flex-col">
            <div className="p-4 border-b border-neutral-200 flex items-center justify-between">
              <h2 className="text-lg font-semibold">แก้ไขแม่แบบอีเมล</h2>
              ...
              <div className="grid grid-cols-1 gap-4">
                <div>
                  ...
                  <input
                    ...
                    placeholder="เช่น booking-confirmed"
                    value={draft.id}
                ...
                <div>
                  ...
                  <textarea
                    ...
                    placeholder="ใช้ {{userName}}, {{serviceName}}, {{date}}, {{amount}}, {{resetLink}}"
                  />
                </div>
                <div className="text-xs text-neutral-600">
                  ตัวแปรที่รองรับ: {'{{userName}}'}, {'{{serviceName}}'}, {'{{date}}'}, {'{{amount}}'}, {'{{resetLink}}'}
                </div>
              </div>
            </div>
